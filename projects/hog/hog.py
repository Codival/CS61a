...
def roll_dice(num_rolls, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    total = 0
    did_they_roll_one = False
    while num_rolls:
        roll_value = dice()
        if roll_value == 1:
            did_they_roll_one = True
        total += roll_value
        num_rolls -= 1
    if did_they_roll_one:
        return 1
    return total
    # END PROBLEM 1
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if player == 0:
            player_0_dice = strategy0(score0, score1)
            result = take_turn(player_0_dice,score1,dice)
            score0 = score0 + result
            if is_swap(score0,score1):
                score0,score1 = score1,score0
        elif player == 1:
            player_1_dice = strategy1(score1,score0)
            result = take_turn(player_1_dice,score0,dice)
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 10
    free_bacon_above_margin = free_bacon(opponent_score) >= margin
    if free_bacon_above_margin:
        return 0
    else:
        return num_rolls
    # END PROBLEM 10
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    # BEGIN PROBLEM 11
    free_bacon_above_margin = free_bacon(opponent_score) >= margin
    will_swap_occur = is_swap(score + free_bacon(opponent_score),opponent_score)
    is_swap_beneficial = will_swap_occur and opponent_score > score
    if is_swap_beneficial or free_bacon_above_margin:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
