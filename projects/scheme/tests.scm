...
(define my-triangle ((triangle-area 5)4))
;expect my-triangle

(/ (+ (* 2 my-triangle) 1) 3)
;expect 7

(define (triangle-area-lambda h)
  (lambda (b) (* .5 b h)))
;expect triangle-area-lambda

((triangle-area-lambda 5) 4)
;expect 10

(define (equal-checker fn1 fn2 x)
  (cond
    ((= 1 x) #t)
    ((= ((fn1 x)(- x 1)) ((fn2 x) (- x 1))) (equal-checker fn1 fn2 (- x 1)))
    (else #f)
  )
)
;expect equal-checker

(equal-checker triangle-area triangle-area-lambda 100)
;expect #t

(define (square-area-lambda h)
  (lambda (b) (* b h)))
;expect square-area-lambda

(equal-checker triangle-area square-area-lambda 2)
;expect #f

(equal-checker triangle-area-lambda square-area-lambda 2)
;expect #f

(if (equal-checker triangle-area-lambda triangle-area 10) 'okay_dokey_artichokey 'uh-oh_spagetti-oh)
;expect okay_dokey_artichokey

(if (equal-checker square-area-lambda triangle-area 10) 'okay_dokey_artichokey 'uh-oh_spagetti-oh)
;expect uh-oh_spagetti-oh

(if (or (equal-checker square-area-lambda triangle-area 10) (equal-checker triangle-area triangle-area-lambda 10))
    'okay_dokey_artichokey 'uh-oh_spagetti-oh)
;expect okay_dokey_artichokey

(if (and (equal-checker square-area-lambda triangle-area 10) (equal-checker triangle-area triangle-area-lambda 10))
    'okay_dokey_artichokey 'uh-oh_spagetti-oh)
;expect uh-oh_spagetti-oh
...
(and #f (/ 1 0) (undefined-function-call 23 5 2342))
;expect #f
