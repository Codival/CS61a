...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        raise NameError("Variable '{0}' cannot be Found".format(self.string))
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        actual_operator = self.operator.eval(env)
        actual_operands = []
        for stuff in self.operands:
            actual_operands.append(stuff.eval(env))
        return actual_operator.apply(actual_operands)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        for param,argue in zip(self.parameters,arguments):
            new_frame[param]=argue
