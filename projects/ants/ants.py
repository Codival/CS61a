...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if not (self.ant.is_container and self.ant.contained_ant and self.ant.contained_ant.is_true_queen) \
                    and not self.ant.is_true_queen :
            # END Problem 13
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        if not self.scared_before:
            if self.reverse and self.place.entrance:
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if not self.is_there_a_queen:
            Insect.is_there_a_queen = True
            Ant.__init__(self,armor)
    ...
    def action(self, colony):
        ...
        if self.is_true_queen:
            ...
            while curr_place:
                if isinstance(curr_place.ant,BodyguardAnt):
                    if not curr_place.ant.is_power_doubled:
                        curr_place.ant.is_power_doubled = True
                        curr_place.ant.damage *= 2
                    if curr_place.ant.contained_ant and not curr_place.ant.contained_ant.is_power_doubled:
                        curr_place.ant.contained_ant.is_power_doubled = True
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        # self.armor -= amount
        if self.armor <= amount and self.is_true_queen:
...
def make_scare(action, bee):
    ...
    def new_action(colony):
        nonlocal action
        if bee.effect['scared'] == 0 and not bee.is_scared:
            Bee.action(bee,colony)
        if not bee.scared_before:
            bee.is_scared = True
            bee.reverse = True
            action(colony)
            if bee.effect['scared'] > 0:
                bee.effect['scared'] -= 1
            if bee.effect['scared'] == 0:
                bee.scared_before = True
                bee.reverse = False
                if bee.effect['slow']:
                    action = make_slow(bee.action,bee)
                    bee.action = action
                    bee.action(colony)
                    bee.is_scared = False
                bee.action = action
...
def apply_effect(effect, bee, duration):
    ...
    "*** YOUR CODE HERE ***"
    if effect == make_slow:
        bee.effect['slow'] += duration
        bee.action = effect(bee.action, bee)
    else:
        if not bee.scared_before and not bee.is_scared:
            bee.effect['scared'] += duration
            bee.action = effect(bee.action,bee)
    # END Problem EC
